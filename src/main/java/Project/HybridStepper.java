...
import static Week05.Sign.ZERO;
import static Week05.SignInterpreter.clone_state;

public class HybridStepper implements AbstractStepper {

    private final Map<String, JSONObject> classes;

    private final int THROW_LEVEL = NULL.toInt();

    public HybridStepper(Map<String, JSONObject> classes) {
        ...
        switch(instruction.getString("opr")) {
            ...
            case "if" -> {
                ...
                if(condition.equals("is")) {
                    if(value1.has("abstract") && value1.get("abstract") instanceof ANull a1 && value2.has("abstract") && value2.get("abstract") instanceof ANull a2) {
                        if(a1 == NULL && a2 == NULL) bools.add(true);
                        else bools.addAll(Set.of(true, false));
                    } else bools.addAll(Set.of(true, false));
                } else if(condition.equals("isnot")) {
                    if(value1.has("abstract") && value1.get("abstract") instanceof ANull a1 && value2.has("abstract") && value2.get("abstract") instanceof ANull a2) {
                        if(a1 != a2) bools.add(true);
                        else bools.addAll(Set.of(true, false));
                    } else bools.addAll(Set.of(true, false));
                } else {
            ...
            case "ifz" -> {
                ...
                Set<Boolean> bools = new HashSet<>();
                if(condition.equals("is")) {
                    if(value.has("abstract") && value.get("abstract") instanceof ANull a) {
                        if(a == NULL) bools.add(true);
                        else bools.addAll(Set.of(true, false));
                    } else bools.addAll(Set.of(true, false));
                } else if(condition.equals("isnot")) {
                    if(value.has("abstract") && value.get("abstract") instanceof ANull a) {
                        if(a == NOTNULL) bools.add(true);
                        else if(a == NULL) bools.add(false);
                        else bools.addAll(Set.of(true, false));
                    } else bools.addAll(Set.of(true, false));
                } else {
                    Function<Sign, Set<Boolean>> fun = (s) -> switch(instruction.getString("condition")) {
                        case "eq"   -> Sign.eq(s, Sign.ZERO);
                        case "ne"   -> Sign.ne(s, Sign.ZERO);
                        case "le"   -> Sign.le(s, Sign.ZERO);
                        case "lt"   -> Sign.lt(s, Sign.ZERO);
                        case "ge"   -> Sign.ge(s, Sign.ZERO);
                        case "gt"   -> Sign.gt(s, Sign.ZERO);
                        default     -> Set.of(true, false);
                    };
            ...
            case "jsr" -> {
                ...
                Frame next = psi.peek();
                JSONObject value = new JSONObject(Map.of("type", "int", "value", next.iota().e2(), "sign", Set.of(Sign.toSign(target))));
                f.sigma().push(value);
            ...
            case "get" -> {
                ...
                f.sigma().push(SimpleType.isNumeric(value.get()) ? SignInterpreter.toAbstract(value.get()) : ANull.toAbstract(value.get())); // TODO: Check type of value and set annotation
                psi.push(new Frame(f.lambda(), f.sigma(), new Pair<>(f.iota().e1(), f.iota().e2() + 1)));
            ...
            case "newarray" -> {
                ...
                for(int i = 0; i < array.length(); i++) {
                    ...
                    if(type instanceof String) {
                        value.put("abstract", NOTNULL);
                        value.put("sign", Sign.ZERO);
                    }
            ...
            case "arraylength" -> {
                ...
                f.sigma().push(new JSONObject(Map.of("type", "int", "value", 0, "abstract", NOTNULL, "sign", Set.of(arraylength > 0 ? POSITIVE : ZERO))));
